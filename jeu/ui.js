...
function formatText(text) {
    ...
    // {partenaire} devient le nom du conjoint, c'est-à-dire le protagoniste
    // que le joueur n'a pas choisi : Mila et Didi forment le couple du récit.
    const conjoint = nomDuPartenaire();

    return text
        .replace(/\{partenaire\}/g, conjoint)
        .replace(/\[([^/\]]*)\/([^\]]*)\]/g, (_, m, f) => isMale ? m : f);
}

/**
 * Nom du conjoint : l'autre protagoniste.
 * @returns {string}
 */
function nomDuPartenaire() {
    return state.user.protagonist === 'Mila' ? 'Didi' : 'Mila';
}
...
function bindScreenActions(onStart, onContinue, onProfileComplete, onProtagonistSelected) {
    ...
    protoScreen.querySelectorAll('.choose-btn').forEach(btn => {
        btn.addEventListener('click', (e) => {
            ...
            // Le conjoint est l'autre protagoniste : Mila et Didi sont en couple.
            state.progress.characters.partner.name = nomDuPartenaire();
            saveGame();
...
/**
 * Bouton de lecture à voix haute, à poser sur les écrans de bilan.
 * Rendu vide si l'appareil n'a pas de synthèse vocale, pour ne pas afficher
 * une commande inopérante.
 * @returns {string}
 */
function getBoutonLectureMarkup() {
    if (!voixDisponible()) return '';
    return '<button id="btn-lire" class="btn-lire" aria-label="Lire à voix haute">🔊</button>';
}

/**
 * Branche le bouton de lecture sur un texte donné.
 * Un second appui interrompt la lecture : c'est le même bouton qui lance et
 * qui arrête, comme sur un lecteur audio.
 * @param {() => string} texte fonction renvoyant le texte à lire
 */
function brancherLecture(texte) {
    const b = getEl('btn-lire');
    if (!b) return;

    const refleter = (actif) => {
        b.classList.toggle('lit', !!actif);
        b.textContent = actif ? '⏸' : '🔊';
    };
    auChangement(refleter);
    refleter(enLecture());

    b.addEventListener('click', () => {
        if (enLecture()) { stopper(); refleter(false); return; }
        lire(texte());
    });
}
...
function showFinDeVie() {
    ...
    const texteFin = () => [
        'La balance a parlé.',
        `${formatText(bilan.titre)}. ${bilan.balanceLevel}.`,
        `Pilier dominant : ${nomDuPilier(bilan.dominantStat)}.`,
        `Pilier négligé : ${nomDuPilier(bilan.weakestStat)}.`,
        `Ce qu'on retient de vous : ${formatText(bilan.primaryReputation)}.`,
        `Votre foyer : ${bilan.familyStatus}.`,
        bilan.isBalanced
            ? "Aucun pilier n'a écrasé les autres. C'est là tout l'art."
            : "Une vie penchée n'est pas une vie ratée, mais la balance s'en souvient."
    ].join(' ');

    lire(texteFin());
    brancherLecture(texteFin);
...
export function updateCurrentEventUI() {
    ...
    // Personnage en pied, présent dans la scène. Sa tenue suit le chapitre :
...
/**
 * Identifiant de la scène déjà tranchée.
 * Sur mobile, un appui peut produire deux événements rapprochés : sans ce
 * garde-fou la décision était enregistrée deux fois, le bilan affichait la
 * même scène en double et ses effets s'appliquaient deux fois.
 *
 * Le verrou porte sur la scène et non sur une durée : un joueur rapide n'est
...
function handleChoice(choice, eventId) {
    ...
    sceneTranchee = eventId;

    // On neutralise aussi les boutons, pour que rien ne réponde plus à l'écran.
    document.querySelectorAll('#dilemma-container .choice-btn')
    ...
    // Le portrait réagit à la nature de la décision.
    reagir(teinteDuChoix(choice.effects));

    // Un personnage peut entrer dans la vie du joueur à cet instant.
    activerPersonnages().forEach(id => {
        const p = state.progress.characters[id];
        if (p && p.name) annoncerPersonnage(p);
    });
...
function showAchievementToast(ach) {
    ...
    toast.innerHTML = `
        ...
        <div class="toast-content">
            <div class="toast-title">${ach.kicker || 'HAUT FAIT DÉBLOQUÉ'}</div>
            <div class="toast-name">${ach.title}</div>
            ${ach.desc ? `<div class="toast-desc">${ach.desc}</div>` : ''}
        </div>
    `;
...
/**
 * Annonce l'entrée d'un personnage dans le récit.
 * Réutilise le bandeau des hauts faits : même grammaire visuelle, donc rien
 * de nouveau à apprendre pour le joueur.
 * @param {{name:string, role:string}} perso
 */
function annoncerPersonnage(perso) {
    showAchievementToast({
        icon: '\u{1F464}',
        kicker: 'NOUVELLE RENCONTRE',
        title: perso.name,
        desc: perso.role
    });
}
...
export async function showChapterIntro(idx) {
    ...
    if (portraitContainer) {
        portraitContainer.innerHTML =
            `<img id="chapter-intro-portrait-img" src="${portrait}" class="chapter-intro-portrait" alt="">`;
    }
...
function showChapterSummary(chapterIdx, echos = []) {
    ...
    getEl('game-scroll-container').innerHTML = `
        <div class="summary-card">
            ...
            <p class="summary-usure">Le temps a passé : chaque pilier a perdu
                ${USURE_PAR_CHAPITRE} points, et les liens que vous ne nourrissez
    ...
    // Lecture du bilan : le verdict, puis chaque décision du chapitre.
    brancherLecture(() => {
        const t = [
            `Fin du chapitre ${chapterIdx + 1}. ${chapitre ? formatText(chapitre.title) : ''}.`,
            `Votre équilibre : ${calculateBalance().level}.`,
                ...
                <div class="tutorial-card home-menu-card" style="text-align: left;">
                    ...
                    <ul class="aide-liste">
                        ...
                        <li>Entre deux chapitres, le temps passe : chaque pilier
                            perd du terrain, et les liens que vous ne nourrissez pas
                            se refroidissent. Rien ne reste acquis.</li>
                        <li>Vos décisions changent vos relations. Certaines personnes
                            n'entrent dans votre vie que si vous leur en laissez
                            la place : consultez la Bibliothèque Vivante.</li>
